...
## <a name='convert-numbers-and-strings'></a> We must convert numbers to strings or vice versa when operating on them.
...
* The older, clunkier approach is to use _%-formatting_:
...
* The `%s` terms tell the interpreter to insert - as strings - the values in brackets (variables in this case) after the `%` following the main string. 
* %-formatting becomes hard to follow when many variables need to be inserted.
* We can use _f-strings_ for a more elegant approach:
